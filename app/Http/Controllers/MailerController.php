...
class MailerController extends Controller {
    ...
    public function notificar(){
        $correos = User::select("email");
        $busquedaEmail=User::where("email","=","danielcarvagil10@gmail.com")->value("email");
        
        
        $productoDesc=Productos::select("descuento")->where("descuento",">","0.1")->get();

        $prodcutoNombre = Productos::select("nombre")->where("descuento",">","0.1")->get();
               // $productoDescuento = Str::replaceFirst('[{"descuento":',':',$productoDesc);
                $productoDescuento = Str::substr($productoDesc,17,-28);
                $prodcutoN = Str::substr($prodcutoNombre,12,-29);

        $productoP=Productos::where("precio","=","50000")->value('precio');
        ...
        require base_path("vendor/autoload.php");
            ...
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port = 587;                          // port - 587/465
            $mail->setFrom($busquedaEmail, 'Tienda Axes');
            $mail->addAddress($busquedaEmail);
            $mail->isHTML(true);                // Set email content format to HTML
                                                    ...
                                                    <tbody>
                                                        <tr style="padding:0;text-align:left;vertical-align:top">
                                                            <th style="Margin:0;color:#322f37;font-family:Helvetica,Arial,sans-serif;font-size:16px;font-weight:400;line-height:1.3;margin:0;padding:0;text-align:left">
                                                                ...
                                                                Tenemos los siguientes productos con descuento: <br><br> <strong>'.$prodcutoN.'</strong> con un descuento del <strong>'.$productoDescuento.'%</strong><br><br>
                                                                ...
                                                                <br><br>
                                                                 ...
                                                                 <br><br><br>
                                                                     ...
                                                                     padding-left:150px;
                                                                     padding-right:150px;
                                                                     background-color:#F69DA1;" href="http://127.0.0.1:8000/Promociones">Mostrar Descuentos</a>
                                                                 </p>
                                                                 <br><br>
                                                                <hr style="height:3px;border:none;background:linear-gradient(274deg, #46c9d0d8, #6262e2d2 44%, #c862f7d8)">
            ...
            $dt = $mail->send();
            return back()->with("success1", "Hemos notificado a todos los usuarios sobre nuestros descuentos mediante un correo electronico.");                           
            }
    }
}